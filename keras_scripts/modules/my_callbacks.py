            ...
            if self.save_best_only:
                ...
                if current is None:
                    ...
                    if self.monitor_op(current, self.best):
                        ...
                        self.best = current
                        if self.save_weights_only:
                            detachmodel(self.model).save_weights(
                                filepath, overwrite=True)
                        else:
                            detachmodel(self.model).save(
                                filepath, overwrite=True)
                    else:
                        if self.verbose > 0:
                            print('Epoch %05d: %s did not improve' %
    ...
    def on_train_begin(self, logs={}):
        ...
        with open(self.fil, 'ar+') as fil:
            if os.stat(self.fil).st_size == 0:
                self.losses.append(
                    ['iter', 'acc', 'loss', 'val_acc', 'val_loss'])
    ...
    def on_batch_end(self, batch, logs={}):
        if self.iteration % 1000 == 0:
            val_loss, val_acc = self.model.evaluate_generator(
                self.validation_generator, steps=self.validation_steps)
            self.losses.append([self.iteration, logs.get(
                'acc'), logs.get('loss'), val_acc, val_loss])
        self.iteration += 1

    def on_epoch_end(self, epoch, logs={}):
        with open(self.fil, 'a') as fil:
            for iteration, acc, loss, val_acc, val_loss in self.losses:
                fil.write(str(iteration) + ' ' + str(acc) + ' ' + str(loss) +
